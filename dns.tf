...
resource "cloudflare_record" "wireguard_dns_ipv4" {
  ...
  value   = hcloud_primary_ip.wireguard_primary_ipv4[count.index].ip_address
  type    = "A"
  ttl     = var.cloudflare_default_ttl
}
...
resource "cloudflare_record" "wireguard_dns_ipv6" {
  ...
  zone_id = data.cloudflare_zone.domain_zone["sirjavik.de"].id
  name    = hcloud_server.wireguard[count.index].name
  value   = "${hcloud_primary_ip.wireguard_primary_ipv6[count.index].ip_address}1"
  type    = "AAAA"
  ttl     = var.cloudflare_default_ttl
}
...
resource "hcloud_rdns" "wireguard_rdns_ipv4" {
  count         = var.server_count["wireguard"]
  primary_ip_id = hcloud_primary_ip.wireguard_primary_ipv4[count.index].id
  ip_address    = hcloud_primary_ip.wireguard_primary_ipv4[count.index].ip_address
  dns_ptr       = hcloud_server.wireguard[count.index].name
}
resource "hcloud_rdns" "wireguard_rdns_ipv6" {
  count         = var.server_count["wireguard"]
  primary_ip_id = hcloud_primary_ip.wireguard_primary_ipv6[count.index].id
  ip_address    = hcloud_primary_ip.wireguard_primary_ipv6[count.index].ip_address
  dns_ptr       = hcloud_server.wireguard[count.index].name
}
