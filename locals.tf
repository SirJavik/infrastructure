...
locals {
  ...
  initServers = merge(
    { for server in hcloud_server.ansible : server.name => { name = server.name, id = server.id, ipv4 = server.ipv4_address, ipv6 = server.ipv6_address } },
    { for server in hcloud_server.mailserver : server.name => { name = server.name, id = server.id, ipv4 = server.ipv4_address, ipv6 = server.ipv6_address } },
    { for server in hcloud_server.webserver : server.name => { name = server.name, id = server.id, ipv4 = server.ipv4_address, ipv6 = server.ipv6_address } },
    { for server in hcloud_server.icinga : server.name => { name = server.name, id = server.id, ipv4 = server.ipv4_address, ipv6 = server.ipv6_address } },
    { for server in hcloud_server.loadbalancer : server.name => { name = server.name, id = server.id, ipv4 = server.ipv4_address, ipv6 = server.ipv6_address } },
  )
}
